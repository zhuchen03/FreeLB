...
class AlbertAttention(BertSelfAttention):
    def __init__(self, config):
        ...
        self.dp_prob = config.attention_probs_dropout_prob
    ...
    def forward(self, input_ids, attention_mask=None, head_mask=None, dp_mask=None):
        ...
        if self.dp_prob > 0 and self.training:
            if dp_mask is None:
                mask1 = torch.zeros_like(attention_probs).bernoulli_(1 - self.dp_prob) / (1 - self.dp_prob)
            else:
                mask1 = dp_mask
        ...
        return (layernormed_context_layer, attention_probs, mask1) if self.output_attentions else (layernormed_context_layer, mask1)
...
class AlbertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, dp_mask=None):
        attention_output = self.attention(hidden_states, attention_mask, head_mask, dp_mask=dp_mask)
        dp_mask_ = attention_output[-1]
        ffn_output = self.ffn(attention_output[0])
        ffn_output = self.activation(ffn_output)
        ffn_output = self.ffn_output(ffn_output)
        hidden_states = self.full_layer_layer_norm(ffn_output + attention_output[0])

        return (hidden_states,) + attention_output[1:], dp_mask_  # add attentions if we output them
...
class AlbertLayerGroup(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, dp_masks=None):
        layer_hidden_states = ()
        layer_attentions = ()

        ret_dp_masks = []
        for layer_index, albert_layer in enumerate(self.albert_layers):
            layer_output, dp_mask_ = albert_layer(hidden_states, attention_mask, head_mask[layer_index],
                                dp_mask=dp_masks[layer_index] if dp_masks is not None else None)
        ...
        return outputs, ret_dp_masks  # last-layer hidden state, (layer hidden states), (layer attentions)
...
class AlbertTransformer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, init_dp=True, dp_masks=None):
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            layer_group_output, dp_masks_ = self.albert_layer_groups[group_idx](
                hidden_states,
                attention_mask,
                head_mask[group_idx * layers_per_group : (group_idx + 1) * layers_per_group],
                dp_masks=dp_masks[i] if dp_masks is not None else dp_masks
            )
            hidden_states = layer_group_output[0]
            ret_dp_masks.append(dp_masks_)
...
class AlbertModel(AlbertPreTrainedModel):
    ...
    def __init__(self, config, **kwargs):
        super(AlbertModel, self).__init__(config, **kwargs)
    ...
    def forward(
        ...
        encoder_outputs, dp_masks = self.encoder(embedding_output, extended_attention_mask, head_mask=head_mask, dp_masks=dp_masks)
...
class AlbertForSequenceClassification(AlbertPreTrainedModel):
    ...
    def __init__(self, config, **kwargs):
        super(AlbertForSequenceClassification, self).__init__(config, **kwargs)
        ...
        self.encoder = self.albert # create a handle for the adversarial training code
    ...
    def forward(
        ...
        outputs, encoder_dp_masks = self.albert(
            input_ids=input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            position_ids=position_ids,
