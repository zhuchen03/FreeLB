...
class RobertaHubInterface(nn.Module):
    ...
    def decode_as_token(self, tokens: torch.LongTensor):
        assert tokens.dim() == 1
        tokens = tokens.numpy()
        if tokens[0] == self.task.source_dictionary.bos():
            tokens = tokens[1:]  # remove <s>
        ...
        doc_mask = eos_mask[1:] & eos_mask[:-1]
        sentences = np.split(tokens, doc_mask.nonzero()[0] + 1)
        sentences = [self.bpe.decode_as_token(self.task.source_dictionary.string(s)) for s in sentences]
        if len(sentences) == 1:
            return sentences[0]
        return sentences
    ...
    def extract_features_from_embed(self, tokens, token_embed, return_all_hiddens=False) -> torch.Tensor:
        # if tokens.dim() == 1:
        #     tokens = tokens.unsqueeze(0)
        if token_embed.dim() == 2:
            import pdb
            pdb.set_trace()
            token_embed = token_embed.unsqueeze(0)
        if token_embed.size(-2) > self.model.max_positions():
            raise ValueError('tokens exceeds maximum length: {} > {}'.format(
                token_embed.size(-2), self.model.max_positions()
            ))
        features, extra = self.model(
            tokens,
    ...
    def predict_from_embed(self, head, tokens, embeds, return_logits = False):
        features = self.extract_features_from_embed(tokens, token_embed=embeds)
        logits = self.model.classification_heads[head](features)
        if return_logits:
            return logits
        return F.log_softmax(logits, dim=-1)

    def extract_attns(self, tokens: torch.LongTensor, return_all_hiddens: bool = False) -> torch.Tensor:
        if tokens.dim() == 1:
            tokens = tokens.unsqueeze(0)
        if tokens.size(-1) > self.model.max_positions():
            raise ValueError('tokens exceeds maximum length: {} > {}'.format(
        ...
        return features, extra['attn_list']  # just the last layer's features

    def predict_attn(self, head: str, tokens: torch.LongTensor, return_logits: bool = False):
        features, attn_list = self.extract_attns(tokens)
        logits = self.model.classification_heads[head](features)
        if return_logits:
            return logits, attn_list
        return F.log_softmax(logits, dim=-1), attn_list
    ...
    def disambiguate_pronoun_scores(self, sentence: str) -> bool:
        """
        Usage::
        ...
        assert hasattr(self.task, 'disambiguate_pronoun'), \
            'roberta.disambiguate_pronoun() requires a model trained with the WSC task.'
        with utils.eval(self.model):
            return self.task.disambiguate_pronoun_scores(self.model, sentence, use_cuda=self.device.type == 'cuda')
