...
class Trainer(object):
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        self._set_seed()
        self.model.train()
        # self.model.decoder.sentence_encoder.token_embed
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    ...
                    if self.args.std_adv:
                        train_step = self.task.std_adv_train_step
                    elif self.args.yopo_steps > 1:
                        train_step = self.task.yopo_train_step
                    else:
                        # using our approach
                        train_step = self.task.freelb_train_step
                    loss, sample_size, logging_output = train_step(
                        sample, self.model, self.criterion, self.optimizer,
                        ignore_grad, self.get_num_updates()
                    )
    ...
    def freeat_train_step(self, samples, dummy_batch=False, raise_oom=False):
        """Do forward, backward and parameter update."""
        if self._dummy_batch is None:
            self._dummy_batch = samples[0]
        ...
        # forward and backward pass
        delta_list = []
        for at in range(self.args.adv_steps):
            self.zero_grad()
            logging_outputs, sample_sizes, ooms = [], [], 0
            for i, sample in enumerate(samples):
                sample = self._prepare_sample(sample)
                ...
                try:
                    with maybe_no_sync():
                        # forward and backward
                        loss, sample_size, logging_output, delta_list[i] = self.task.freeat_adv_train_step(
                            sample, self.model, self.criterion, self.optimizer,
                            ignore_grad, delta_list[i], init_dp=True
                        )
