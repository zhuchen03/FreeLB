...
@register_task('qnli_ranking')
class QNLIRankingTask(FairseqTask):
    ...
    def freelb_train_step(self, sample, model, criterion, optimizer, ignore_grad=False, num_updates=0):
        ...
        if self.args.rand_init_mag > 0 and num_updates >= self.args.adv_begin_iter:
            ...
            for idx in range(self.args.num_classes):
                ...
                mag = self.args.rand_init_mag / torch.sqrt(torch.Tensor(sample[input_name]['src_lengths']).to(delta))
        ...
        for t_adv in range(self.args.adv_steps):
            ...
            for dgrad in delta_grad_list:
                denorm += torch.sum(dgrad.view(dgrad.size(0), -1)**2, dim=1).view(-1, 1, 1)
            denorm = torch.sqrt(denorm).detach()
    ...
    def load_dataset(self, split, epoch=0, combine=False, data_path=None, return_only=False, **kwargs):
        ...
        with open(data_path) as h:
            for line in h:
                ...
                question = example['ques']
                # format: `<s> Q: Where would I not want a fox? </s> A: hen house </s>`
                question = 'Q: ' + question
                question_toks = binarize(question, append_bos=True)

                src0 = 'A: ' + example['sent0']
                src_bin0 = torch.cat([question_toks, binarize(src0)])
                src_tokens[0].append(src_bin0)
                src_lengths[0].append(len(src_bin0))

                src1 = 'A: ' + example['sent1']
                src_bin1 = torch.cat([question_toks, binarize(src1)])
                src_tokens[1].append(src_bin1)
                src_lengths[1].append(len(src_bin1))
